...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		for (int i = 1; i < num_procesos; i++)
		{
			...
			for (int j = 0; j < tam; j++)
			{ // para cada fila
				if (j % (num_procesos - 1) + 1 == origen)
				{ // si dicha fila le corresponde al proceso (entre el P01 y P0numprocesos -1)
...
float fila_por_columna(float *matriz_1, int fila, float *matriz_2, int columna, int tam)
{
	float res = 0.0;

	for (int i = 0; i < tam; i++)
	{
		res += matriz_1[fila * tam + i] * matriz_2[i * tam + columna];
	}
...
float *multiplicar_matrices(float *matriz_1, float *matriz_2, int dimension, int num_procesos, int rango_proceso)
{
	...
	{ // para cada fila
		if (i % (num_procesos - 1) + 1 == rango_proceso)
		{ // si dicha fila le corresponde al proceso (entre el P01 y P0numprocesos -1)
			for (int j = 0; j < dimension; j++)
			{ // Calcular para cada columna
				resultado[dimension * i + j] = fila_por_columna(matriz_1, i, matriz_2, j, dimension);
			}
		}
	}
	return resultado;
